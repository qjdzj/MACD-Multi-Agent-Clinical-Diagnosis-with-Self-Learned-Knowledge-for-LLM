...
def create_prompt(
    tags, tool_names, add_tool_descr, tool_use_examples,diagnostic_criteria
) -> PromptTemplate:
...
def build_agent_executor_ZeroShot(
    ...
    if diagnostic_criteria:
        char_to_criteria = {
            ...
            "f": DIAGNOSTIC_CRITERIA_FULL,
        }
        for char in diagnostic_criteria:
            diagnostic_criteria_list.append(char_to_criteria[char])
    diagnostic_criteria = "\n".join(diagnostic_criteria_list)
    ...
    prompt = create_prompt(tags, tool_names, add_tool_descr, tool_use_examples,diagnostic_criteria)
